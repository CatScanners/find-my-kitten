...
Streamer::Streamer(bool stream_to_gpu_pointer) : stream_to_gpu_pointer(stream_to_gpu_pointer){
    init_success = init();
}
...
bool Streamer::init_map(){
    for (__u32 i = 0; i < n_buffers; i++) {
        ...
        if (buffers[i].size() != buf.length) {
            ...
            std::cout << "Expecting image with total size of bytes = " << buffers[i].size() << std::endl;
            std::cout << "Recieved  image with total size of bytes = " << buf.length             << std::endl;
            return false;
        }
...
bool Streamer::init_start_stream(){
    // --- Start streaming ---
...
void Streamer::record_new_image() {
...
v4l2_buffer Streamer::get_v4l2_buffer(int index) {
    v4l2_buffer buf;
