...
// OPTIMAL means minimum squared distance of each feature on camera.
...
DroneState locateDrone(
    const std::vector<vector3D>& positions, const std::vector<vector2D>& features, const DroneState previousState, const bool display = false
);
