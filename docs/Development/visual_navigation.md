...
## Launch Files
The package includes several launch files for different configurations. The launch files don't contain the full lists of parameters, for those consult the oness listed from the node section below. Generally though launch files are the preferred way of launching the nodes.

### stereo.launch.py
Launches the stereo_publisher node with configurable parameters for the stereo camera setup.

### vslam.launch.py
Launches the vSLAM pipeline without the stereo publisher, assuming camera input from elsewhere. This can be used for example when playing back a rosbag of a recorded flight data of depth camera. This does NOT start the vslam_message_transform node.

### stereo_vslam.launch.py
Launches both stereo_publisher and vSLAM components together for integrated stereo vision and SLAM.
This is the simplest option to use, as it launches both at the same time.
...
### stereo_publisher
Custom wrapper for the OAK-D pro camera. The default wrappers had all sorts of unnecessary features running which caused a brownout so we wrote a minimalistic wrapper (brownout could possibly be solved by using the Y adapter). Publishes stereo camera feed, and IMU data using the DepthAI (Luxonis) OAK-D camera. For the full list of launch parameters and other examples and API documentation refer to https://docs.luxonis.com/software/ros/depthai-ros/ (also includes how to use the other features of the camera)
